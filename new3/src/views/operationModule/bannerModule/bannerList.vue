...
<template>
  <div class="v-wrap">
    <div class="v-other-btn-list" v-if="btnVisible && btnVisible.add">
      <el-button class="v-button" type="primary" icon="el-icon-plus" plain @click="toShowEditDialog(1)">添加banner</el-button>
    </div>
    <el-table class="v-table" border v-loading="mainLoading" :data="dataList">
      ...
      <el-table-column label="banner名称" prop="name" align="center"></el-table-column>
      <el-table-column label="图片" prop="name" align="center">
        <template slot-scope="scope">
          <img :src="scope.row.img" width="100%" height="100">
        </template>
      </el-table-column>
      <el-table-column label="所属定位" prop="locationName" align="center"></el-table-column>
    ...
    <!-- 新增，编辑轮播弹窗 -->
    <bannerEditDialog v-model="editInfo.visible" :infoObj="editInfo.infoObj" :type="editInfo.type" @success="getList"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getList () {
      ...
      let res = await this.$post('system/banner/getPage', reqData)
      ...
      this.$confirm('确定删除该banner？', '提示', {
        ...
        beforeClose: async (action, instance, done) => {
          ...
          if (action === 'confirm') {
            ...
            let res = await this.$post('system/banner/delete', { id })
