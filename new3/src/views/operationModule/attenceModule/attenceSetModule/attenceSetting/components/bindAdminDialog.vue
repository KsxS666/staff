...
export default {
  ...
  methods: {
    ...
    async getAdminList () {
      ...
      if (res.code === 0) {
        this.adminList = res.data.filter(ele => {
          return !ele.equipmentCode
        })
      }
    },
    // 获取当前小区已绑定的管理员
    async getCurrentAdmin () {
      ...
      let res = await this.$post('community/communityAdmin/getList', { equipmentId: this.infoObj.id })
    ...
    async saveData () {
      let req = {
        ...
        equipmentId: this.infoObj.id,
        equipmentCode: this.infoObj.equipmentCode,
        equipmentProperty: this.equipmentProperty
      }
      this.saveLoading = true
      console.log(req)
      let res = await this.$post('community/communityAdmin/bindByEquipment', req)
