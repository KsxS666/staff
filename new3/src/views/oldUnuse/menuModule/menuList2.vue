    ...
    <div class="v-tree-table">
      ...
      <el-tree :data="dataList" node-key="id" class="menuTreeWrap" v-loading="mainLoading">
        <div class="v-tree-item" slot-scope="{ node, data }">
          ...
          <div class="center" @click.stop="">{{data.type === 0 ? '菜单' : '按钮'}}</div>
          <div class="center" @click.stop="">
            <el-button plain size="mini" :disabled="node.level >= 3" @click="toShowEditDialog(4, data)">添加下级菜单</el-button>
            <el-button plain size="mini" type="primary" @click="toShowEditDialog(2, data)">编辑</el-button>
            <el-button plain size="mini" type="danger" @click="toDelete(data.id)">删除</el-button>
          </div>
          <!--</div>-->
        </div>
      </el-tree>
