<template>
  <div class="box">
    <h4 class="title">垃圾投放情况</h4>
    <div class="numBox">
      <div>
        <strong>{{ topData.currentMonthTotalCastWeight || 0 }}<i></i></strong>
        <span>本月总投放量 (kg)</span>
      </div>
      <div>
        <strong>{{ topData.yesterdayTotalCastWeight || 0 }}<i></i></strong>
        <span>昨日投放量 (kg)</span>
      </div>
      <div class="right">
        <strong>{{ topData.currentMonthTotalCastIntegral || 0 }}<i></i></strong>
        <span>本月总积分</span>
      </div>
    </div>
    <div class="tipBox">
    ...
    <div id="wasteLaunchChart" v-loading="mainLoading" style=" display: flex; justify-content: center; align-items: center; height: 200px"></div>
  </div>
</template>
...
export default {
  ...
  mounted () {
    this.myChart = this.$charts.init(document.getElementById('wasteLaunchChart'), null, { renderer: 'svg' })
  ...
  methods: {
    async getList () {
      ...
      let res = await this.$post('integral/overview/getCastDetailReport', reqData)
    ...
    onPick (date) {
      ...
      //   if (date.maxDate > new Date()) {
      //     // console.log(date.maxDate, new Date())
      //     this.$message.warning('时间不能大于今天')
      //     this.$nextTick(() => {
    ...
    timeChange () {
      if (this.formData.timeArr && this.formData.timeArr.length) {
        ...
        //   this.$message.warning('时间不能大于今天')
        //   this.$nextTick(() => {
        //     this.formData.timeArr = this.formData.oldTimeArr ? [...this.formData.oldTimeArr] : []
        //   })
        //   return
        // }
        ...
        this.formData.oldTimeArr = this.formData.timeArr ? [...this.formData.timeArr] : []
      }
      this.getList()
    },
    // 渲染图表
      ...
      let option = {
        ...
        tooltip: {
          ...
          formatter: (params) => { // 自定义显示
            ...
            params.map(item => {
              formatterStr += `<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:#24bf72;"></span> 积分：${item.value}<br>`
            })
            return formatterStr
          }
        },
            ...
            itemStyle: {
              normal: {
                ...
                color: new this.$charts.graphic.LinearGradient(
                  0, 0, 0, 1,
                  [
                    { offset: 0, color: '#24bf72' },
                    { offset: 1, color: '#26d575' }
                  ]
                )
              }
            },
...
<style scoped lang="scss" rel="stylesheet/scss">
  ...
  .numBox{
    div:nth-of-type(1){
      i{
        background-image: url("../assets/icon-wasteLaunch1.png");
      }
    }
    div:nth-of-type(2){
      i{
    ...
    div:nth-of-type(3){
      i{
        background-image: url("../assets/icon-wasteLaunch2.png");
      }
    }
  }
  .tipBox{
